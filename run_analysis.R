...
setwd("/cnN0dWRpby1wYmVuaXdhbC1neGd1eQ==/pbeniwal/Pbeniwal/Coursera/Week4/Project/UCI HAR Dataset");
#getwd()
...
testSet = cbind(yTest,subject_Test,xTest);

# Combine Training Data Set & Test Data Set into one Merged Data Set
...
##---- 2. Extract only the measurements on the mean & st&ard deviation for each measurement
...
##---- 5. Create second, independent tidy data set with the average of each variable for each activity & each subject.

# Averaging each activity & each subject as Tidy Data
tidy_Data <- aggregate(MergedDataSet[,names(MergedDataSet) 
                                    != c('activityId','subjectId')],by=list
                      (activityId=MergedDataSet$activityId,
                        subjectId=MergedDataSet$subjectId),mean);

# Export tidyData set 
write.table(tidy_Data, './FinalTidyData.txt',row.names=FALSE,sep='\t')